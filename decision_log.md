...
12. **Golden-set labels are LLM-suggested, human-confirmed** — not fully hand-typed from scratch, and not fully automated either. `suggest_golden_labels.py` pre-fills a `suggested_intent` column via the real classifier; it's kept in a separate column from `true_intent`, which is a distinct human decision you make per row. This is a labeling-speed decision, not a shortcut on the ground truth itself.
